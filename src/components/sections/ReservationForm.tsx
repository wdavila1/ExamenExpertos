...
const ReservationForm = () => {
  ...
  if (isSubmitted) {
    return (
      <div className='flex flex-col items-center justify-center text-center p-8 bg-card border border-border/80 rounded-2xl shadow-md max-w-xl mx-auto space-y-6 animate-fade-in'>
        ...
        <div className='space-y-2'>
          <h3 className='text-2xl font-bold text-foreground font-serif'>¡Reservación Exitosa!</h3>
          <p className='text-muted-foreground'>
            Gracias por elegir <span className='font-semibold text-primary'>Sabores del Valle</span>. Hemos recibido tu solicitud de reservación.
          </p>
        </div>

        <div className='w-full text-left bg-muted/60 p-5 rounded-xl text-sm border border-border/40 space-y-3'>
          ...
          <div className='flex justify-between border-b border-border/60 pb-2'>
            ...
            <span className='font-semibold text-foreground'>{formData.date} a las {formData.time}</span>
          </div>
          <div className='flex justify-between border-b border-border/60 pb-2'>
            <span className='font-medium text-muted-foreground'>Personas:</span>
            <span className='font-semibold text-foreground'>{formData.guests} personas</span>
          </div>
          <div className='flex justify-between pb-1'>
            <span className='font-medium text-muted-foreground'>Confirmación enviada a:</span>
            <span className='font-semibold text-foreground'>{formData.email}</span>
          </div>
        </div>

        <p className='text-xs text-muted-foreground'>
          *Te enviaremos un correo electrónico de confirmación o te contactaremos al número {formData.phone} en los próximos 15 minutos para formalizar tu reservación.
        </p>
  ...
  return (
    <form onSubmit={handleSubmit} className='bg-card border border-border/80 p-6 sm:p-8 rounded-2xl shadow-sm space-y-6 max-w-2xl mx-auto'>
      <div className='space-y-2 text-center lg:text-left'>
        <h3 className='text-2xl font-bold font-serif text-foreground'>Reserva tu Mesa</h3>
        <p className='text-muted-foreground text-sm'>
          Completa el siguiente formulario para asegurar tu lugar. Reservas válidas con al menos 2 horas de anticipación.
        </p>
      </div>
      ...
      <div className='grid gap-6 sm:grid-cols-2'>
        ...
        <div className='space-y-2'>
          <Label htmlFor='phone' className='flex items-center gap-1.5'>
            <Phone className='h-4 w-4 text-muted-foreground' /> Teléfono / Celular
          </Label>
          <Input
            id='phone'
            name='phone'
        ...
        <div className='space-y-2'>
          <Label htmlFor='guests' className='flex items-center gap-1.5'>
            <Users className='h-4 w-4 text-muted-foreground' /> Cantidad de Personas
          </Label>
          <select
            id='guests'
            name='guests'
            ...
            onChange={handleChange}
            className='flex h-9 w-full rounded-md border border-input bg-transparent px-3 py-1 text-base shadow-xs transition-colors outline-none focus-visible:ring-3 focus-visible:ring-ring/50 focus-visible:border-ring dark:bg-input/30 md:text-sm'
          >
            ...
            <option value='11+' className='bg-background text-foreground'>Más de 10 personas</option>
          </select>
        </div>
        ...
        <div className='space-y-2'>
          <Label htmlFor='date' className='flex items-center gap-1.5'>
            <Calendar className='h-4 w-4 text-muted-foreground' /> Fecha
          </Label>
          <Input
            id='date'
            name='date'
        ...
        <div className='space-y-2'>
          ...
          >
            ...
            {['09:00 AM', '09:30 AM', '10:00 AM', '10:30 AM', '11:00 AM', '11:30 AM', '12:00 PM', '12:30 PM', '1:00 PM', '1:30 PM', '2:00 PM', '2:30 PM', '3:00 PM', '3:30 PM', '4:00 PM', '4:30 PM', '5:00 PM', '5:30 PM', '6:00 PM', '6:30 PM', '7:00 PM', '7:30 PM', '8:00 PM', '8:30 PM', '9:00 PM'].map(t => (
